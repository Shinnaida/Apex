...
public partial class MovingMathPage : ContentPage
{
    bool _isHowToPlayOpen;
    bool _isAnimating;

    public MovingMathPage()
    {
        InitializeComponent();
    }
    ...
    protected override void OnAppearing()
    {
        ...
        int bestScore = BrainScoreService.GetGamePerformance("moving_math")?.BestScore ?? 0;
        BestScoreLabel.Text = bestScore.ToString();
        BestRankLabel.Text = MovingMathProgress.ResolveRank(bestScore);
        ChallengeLabel.Text = MovingMathProgress.ResolveChallenge(bestScore);
    }
